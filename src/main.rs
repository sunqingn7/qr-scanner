...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Check for select-screen mode (spawned by overlay)
    let args: Vec<String> = std::env::args().collect();
    if args.len() >= 3 && args[1] == "--select-screen" {
        let result_file = &args[2];
        run_selection_overlay(result_file);
        return Ok(());
    }
...
fn run_selection_overlay(result_file: &str) {
    eprintln!("Selection overlay started, result file: {}", result_file);
    
    let screens = match screenshots::Screen::all() {
        Ok(s) => s,
        Err(e) => {
            let _ = std::fs::write(result_file, format!("ERROR:Failed to get screens: {}", e));
            return;
        }
    };

    if screens.is_empty() {
        let _ = std::fs::write(result_file, "ERROR:No screens found");
        return;
    }
...
impl eframe::App for SelectionApp {
        fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
            ...
            // Make overlay background fully transparent
            let overlay_painter = ctx.layer_painter(egui::LayerId::background());
            overlay_painter.rect_filled(screen_rect, 0.0, egui::Color32::from_rgba_premultiplied(0, 0, 0, 0));
                        ...
                        for screen in &self.screens {
                            ...
                            if x + w as i32 > sx && x < sx + sw && y + h as i32 > sy && y < sy + sh {
                                ...
                                if capture_w > 0 && capture_h > 0 {
                                    if let Ok(img) = screen.capture_area(capture_x, capture_y, capture_w, capture_h) {
                                        let temp_dir = std::env::temp_dir();
                                        let img_path = temp_dir.join("qr_selection.png");
                                        
                                        if img.save(&img_path).is_ok() {
                                            let _ = std::fs::write(&self.result_file, format!("FILE:{}", img_path.display()));
                                        } else {
                                            let _ = std::fs::write(&self.result_file, "ERROR:Failed to save image");
                                        }
                                    } else {
                                        let _ = std::fs::write(&self.result_file, "ERROR:Failed to capture screen");
                                    }
                                }
                                break;
                            }
            ...
            if input.key_pressed(egui::Key::Escape) {
                let _ = std::fs::write(&self.result_file, "CANCELLED");
                ctx.data_mut(|d| d.remove_temp::<SelectionState>(egui::Id::new("selection")));
                ctx.send_viewport_cmd(egui::ViewportCommand::Close);
            }
    ...
    let native_options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_inner_size([(max_x - min_x) as f32, (max_y - min_y) as f32])
            .with_position(egui::Pos2::new(min_x as f32, min_y as f32))
